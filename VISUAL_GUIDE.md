# Visual Guide: Department, Team & Reports System

## 🎨 Organization Structure Diagram

### Your New Organization

```
┌─────────────────────────────────────────────────────────────────┐
│                        COMPANY                                  │
│                                                                 │
│  ┌──────────────────────┐  ┌──────────────────────┐            │
│  │  DEPARTMENT: IT      │  │  DEPARTMENT: HR      │  ┌────────┐│
│  │  (Manager: Ahmad)    │  │  (Manager: Test)     │  │  DEPT: ││
│  │                      │  │                      │  │  OPS   ││
│  │  ┌────────┐ ┌──────┐│  │  ┌──────────┐        │  │        ││
│  │  │BACKEND │ │FRONT-│  │  │RECRUIT   │  ┌────┐│  │ ┌────┐ ││
│  │  │Team    │ │END   │  │  │Team      │  │PAY-│  │ │CUST││ ││
│  │  │        │ │Team  │  │  │          │  │ROLL│  │ │SUP-││ ││
│  │  │        │ │      │  │  │          │  │    │  │ │PORT││ ││
│  │  │Lead:   │ │Lead: │  │  │Lead:     │  │    │  │ │    │ ││
│  │  │Ahmad   │ │Siti  │  │  │Test      │  │Lead│  │ │    │ ││
│  │  │        │ │      │  │  │          │  │None│  │ │    │ ││
│  │  └────────┘ └──────┘  │  └──────────┘  └────┘  │ └────┘ ││
│  │                      │  │                      │  │        ││
│  └──────────────────────┘  └──────────────────────┘  └────────┘│
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📊 Database Relationship Diagram

```
┌──────────────────────────────────────┐
│         DEPARTMENTS TABLE            │
│                                      │
│ PK: department_id                    │
│ • department_name (UNIQUE)           │
│ • department_code (UNIQUE)           │
│ • manager_id (FK → Staff)            │
│ • location                           │
│ • status: active/inactive            │
└──────────────┬───────────────────────┘
               │
               │ 1 Department → Many Teams
               │
               ▼
┌──────────────────────────────────────┐
│           TEAMS TABLE                │
│                                      │
│ PK: team_id                          │
│ • team_name                          │
│ • team_code (UNIQUE)                 │
│ • department_id (FK) ─────┐          │
│ • team_lead_id (FK → Staff)          │
│ • status: active/inactive            │
│ • UNIQUE(team_name,dept_id)          │
└──────────────┬───────────────────────┘
               │
               │ 1 Team → Many Staff
               │
               ▼
┌──────────────────────────────────────┐
│          STAFF TABLE                 │
│                                      │
│ PK: staff_id                         │
│ • staff_name                         │
│ • staff_email (UNIQUE)               │
│ • staff_password                     │
│ • department_id (FK) ────────┐       │
│ • team_id (FK) ─────┐        │       │
│                     │        │       │
└──────────────┬──────┴────────┴───────┘
               │
               │ 1 Staff → Many Attendance Records
               │
               ▼
┌──────────────────────────────────────┐
│      ATTENDANCE TABLE                │
│                                      │
│ • staff_id (FK)                      │
│ • attendance_date                    │
│ • check_in_time                      │
│ • check_out_time                     │
│ • status: present/absent/late/leave  │
│ • remarks                            │
└──────────────────────────────────────┘

                    REPORTS SECTION
┌──────────────────────────────────────┐
│   ATTENDANCE_REPORTS TABLE           │
│                                      │
│ PK: report_id                        │
│ • admin_id (FK) – who created        │
│ • report_name                        │
│ • report_type: dept/team/staff/sum   │
│ • department_id (FK, optional)       │
│ • team_id (FK, optional)             │
│ • staff_id (FK, optional)            │
│ • start_date, end_date               │
│ • Calculated fields:                 │
│   - present_days                     │
│   - absent_days                      │
│   - late_days                        │
│   - leave_days                       │
│   - attendance_percentage            │
│ • generated_at                       │
└──────────────┬───────────────────────┘
               │
               │ 1 Report → Many Details
               │
               ▼
┌──────────────────────────────────────┐
│ ATTENDANCE_REPORT_DETAILS TABLE      │
│                                      │
│ • report_id (FK) – parent report     │
│ • staff_id (FK)                      │
│ • attendance_id (FK, optional)       │
│ • attendance_date                    │
│ • check_in_time, check_out_time      │
│ • status                             │
│ • duration_minutes                   │
│ • remarks                            │
│                                      │
│ Purpose: Snapshot of data when       │
│ report was generated                 │
└──────────────────────────────────────┘
```

---

## 🔄 Data Flow Diagram

### Staff Attendance Recording

```
┌─────────────────┐
│  Staff Login    │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Click Check-In  │
└────────┬────────┘
         │
         ▼
┌─────────────────────────────────────┐
│  CREATE Attendance Record           │
│  • staff_id (from session)          │
│  • attendance_date (TODAY)          │
│  • check_in_time (NOW)              │
│  • status: 'present'                │
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────┐
│ Work Day...     │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Click Check-Out │
└────────┬────────┘
         │
         ▼
┌─────────────────────────────────────┐
│  UPDATE Attendance Record           │
│  • check_out_time (NOW)             │
│  • Calculate duration               │
│  • Finalize status                  │
└────────┬────────────────────────────┘
         │
         ▼
┌────────────────────────────────────┐
│ Attendance recorded for today      │
└────────────────────────────────────┘
```

### Report Generation Process

```
┌─────────────────────────────┐
│  Admin Request Report       │
│  • Select type: DEPARTMENT  │
│  • Select: IT Department    │
│  • Start: 2025-11-01        │
│  • End: 2025-11-30          │
└────────────┬────────────────┘
             │
             ▼
┌──────────────────────────────────────┐
│  Query Data                          │
│  1. Find IT Department               │
│  2. Get all teams in IT              │
│  3. Get all staff in IT teams        │
│  4. Get attendance records           │
│  5. Calculate statistics             │
└────────────┬─────────────────────────┘
             │
             ▼
┌──────────────────────────────────────┐
│  CREATE Report Record                │
│  • report_type: 'department'         │
│  • department_id: IT_ID              │
│  • Statistics: present, absent, etc. │
│  • generated_at: NOW()               │
└────────────┬─────────────────────────┘
             │
             ▼
┌──────────────────────────────────────┐
│  CREATE Report Details (per staff)   │
│  For each staff member:              │
│  • For each day in date range:       │
│    - Create detail record            │
│    - Copy attendance data            │
│    - Store in report_details table   │
└────────────┬─────────────────────────┘
             │
             ▼
┌──────────────────────────────────────┐
│  Report Completed & Stored           │
│  • Available for future reference    │
│  • Can be printed/exported           │
│  • Historical data preserved         │
└──────────────────────────────────────┘
```

---

## 📈 Query Flow Diagrams

### Query 1: Get All IT Department Staff

```
User Request: "Show IT department staff"
       │
       ▼
┌──────────────────────────────┐
│ Department::where('code'='IT')
│           ->first()          │
└──────────┬───────────────────┘
           │ Returns: IT Department record
           ▼
┌──────────────────────────────┐
│ $dept->staff()               │
│ (Eloquent Relationship)      │
└──────────┬───────────────────┘
           │ Uses: department_id FK
           │ Queries: staff table
           ▼
┌──────────────────────────────┐
│ Returns: Collection of Staff │
│ • Ahmad Hassan              │
│ • Siti Nurhaliza            │
└──────────────────────────────┘
```

### Query 2: Get Backend Team Members with Details

```
User Request: "Show Backend team members"
       │
       ▼
┌──────────────────────────────────────┐
│ Team::where('code'='IT-BACKEND')     │
│      ->with('staff.profile')         │
│      ->first()                       │
└──────────┬────────────────────────────┘
           │ 
           ▼ Returns: Team record + Staff
┌──────────────────────────────────────┐
│ $team->staff                         │
│ Contains: All Backend team members   │
│ Including: Staff profiles (eager)    │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ Returns Collection:                  │
│ • Ahmad Hassan - Backend Developer   │
│ • Phone: 012-xxxx-xxxx               │
│ • Position: Senior Developer         │
└──────────────────────────────────────┘
```

### Query 3: Generate Department Report

```
User: Admin → Click "Generate IT Report"
       │
       ▼
┌──────────────────────────────────────┐
│ Get Department: IT                   │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ Get All Teams in IT                  │
│ • Backend                            │
│ • Frontend                           │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ Get All Staff in Teams               │
│ • Ahmad (Backend)                    │
│ • Siti (Frontend)                    │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ Get Attendance (Nov 1-30)            │
│ • Query: attendance table            │
│ • Filter: staff_id IN (Ahmad, Siti) │
│ • Filter: date range                │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ Calculate Statistics                 │
│ • Present Days: 20                   │
│ • Absent Days: 0                     │
│ • Late Days: 2                       │
│ • Leave Days: 0                      │
│ • Percentage: 91%                    │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ CREATE Report Record                 │
│ • Type: department                   │
│ • Statistics stored                  │
│ • Timestamp: now                     │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ CREATE Report Details                │
│ For each staff member × each day:    │
│ • Ahmad - Nov 1: present 09:00-17:00│
│ • Ahmad - Nov 2: absent              │
│ • Siti - Nov 1: present 09:00-17:00 │
│ • ... etc                            │
└──────────┬────────────────────────────┘
           │
           ▼
┌──────────────────────────────────────┐
│ ✅ Report Complete & Stored          │
│ Report ID: 1                         │
│ 22 detail records created            │
└──────────────────────────────────────┘
```

---

## 🎯 Use Case Scenarios

### Scenario 1: New Employee Joins Backend Team

```
Action: Add Ahmad to Backend Team

1. Create Staff Record
   ├── staff_id: auto
   ├── name: Ahmad Hassan
   ├── email: ahmad@utm.edu.my
   ├── department_id: 1 (IT)
   └── team_id: 1 (Backend)

2. System automatically:
   ├── Links to IT Department
   ├── Links to Backend Team
   ├── Makes eligible for team reports
   ├── Enables attendance tracking
   └── Shows in department views

Result: Ahmad appears in:
├── IT Department staff list
├── Backend Team member list
├── Attendance tracking
└── Department/Team reports
```

### Scenario 2: Department Manager Reviews Team Performance

```
Action: IT Manager → View Backend Team Report

1. Query Flow:
   Department (IT)
   └─→ Teams [Backend, Frontend]
       └─→ Attendance (Nov 1-30)
           ├── Ahmad: 20 present, 2 late
           └── 3 other Backend team members

2. System calculates:
   ├── Team presence rate: 87%
   ├── Days with all present: 15
   ├── Days with absences: 2
   ├── Trends: improving/declining

3. Report Generated:
   ├── Team-level statistics
   ├── Individual breakdowns
   ├── Comparison to other teams
   ├── Exportable PDF/Excel
```

### Scenario 3: HR Needs Organization-Wide Report

```
Action: Admin → Generate Summary Report

1. Query across all departments:
   ├── IT Department
   │  ├── Backend: 87% present
   │  └── Frontend: 92% present
   ├── HR Department
   │  ├── Recruitment: 95% present
   │  └── Payroll: 89% present
   └── Operations: 84% present

2. Organization Summary:
   ├── Total Staff: 3 (test data)
   ├── Overall Present: 89.3%
   ├── Best Department: HR (92%)
   ├── Needs Attention: Operations (84%)

3. Historical Comparison:
   └─ Compare to previous month
```

---

## 🔄 Relationship Paths

### Path 1: From Department to Attendance

```
Department
  └─ .teams()
      └─ .staff()
          └─ .attendance()

Example:
$dept = Department::find(1); // IT
$attendance = $dept->teams()   // Get teams
                 ->get()
                 ->pluck('staff') // Get staff in each team
                 ->flatten()      // Flatten collection
                 ->pluck('attendance') // Get attendance
                 ->flatten();
```

### Path 2: From Team Lead to Team Members

```
Staff (as Team Lead)
  └─ .teamsManaged()
      └─ .staff()
          └─ .attendance()

Example:
$lead = Staff::find(1); // Ahmad
$teamMembers = $lead->teamsManaged() // Get teams led
                      ->get()
                      ->pluck('staff')
                      ->flatten();
```

### Path 3: From Report to Individual Records

```
AttendanceReport
  └─ .details()
      └─ .staff()

Example:
$report = AttendanceReport::find(1);
$details = $report->details()
                  ->with('staff', 'attendance')
                  ->get();

foreach ($details as $detail) {
    echo $detail->staff->name . ": " . $detail->status;
}
```

---

## 📊 Report Types Visual

```
┌─────────────────────────────────────────┐
│     REPORT TYPES                        │
├─────────────────────────────────────────┤
│                                         │
│  1. STAFF REPORT (Individual)           │
│     ├─ Staff: Ahmad Hassan             │
│     ├─ Period: Nov 1-30                │
│     ├─ Present: 20 days                │
│     ├─ Absent: 0 days                  │
│     └─ Percentage: 95%                 │
│                                        │
│  2. TEAM REPORT                        │
│     ├─ Team: Backend Development       │
│     ├─ Department: IT                  │
│     ├─ Members: 5                      │
│     ├─ Overall Present: 87%            │
│     └─ Details per member shown        │
│                                        │
│  3. DEPARTMENT REPORT                  │
│     ├─ Department: IT                  │
│     ├─ Teams: 2 (Backend, Frontend)    │
│     ├─ Total Staff: 7                  │
│     ├─ Overall Present: 89%            │
│     └─ Stats by team shown             │
│                                        │
│  4. SUMMARY REPORT                     │
│     ├─ Organization-wide               │
│     ├─ Total Staff: 20                 │
│     ├─ Departments: 3                  │
│     ├─ Overall Present: 87%            │
│     └─ Trends & comparisons            │
│                                        │
└─────────────────────────────────────────┘
```

---

## 🎨 Dashboard Views

### Admin Dashboard View

```
┌────────────────────────────────────────────────┐
│  ADMIN DASHBOARD                               │
├────────────────────────────────────────────────┤
│                                                │
│  Quick Stats                                  │
│  ├─ Total Staff: 20                          │
│  ├─ Present Today: 18 (90%)                  │
│  ├─ Absent: 2                                │
│  └─ Late: 0                                  │
│                                              │
│  Department Overview                         │
│  ├─ IT [█████████░] 89%                      │
│  ├─ HR [██████████] 92%                      │
│  └─ OPS [████████░░] 84%                     │
│                                              │
│  Quick Actions                               │
│  ├─ [Generate Report]                        │
│  ├─ [Manage Departments]                     │
│  ├─ [Manage Teams]                           │
│  └─ [View History]                           │
│                                              │
└────────────────────────────────────────────────┘
```

---

## ✅ Summary of Visual Changes

| View | Before | After |
|------|--------|-------|
| Staff Organization | Flat list | Hierarchical (Dept→Team→Staff) |
| Filtering | By individual only | By staff/team/dept |
| Reports | Staff reports only | Staff/Team/Dept/Summary reports |
| Management | Limited | Full department & team mgmt |
| Relationships | 1 layer | Multi-layer hierarchy |

---

**Version:** 2.0  
**Date:** November 20, 2025  
**Visual Guide Complete** ✅
